...
- **Issue #99 completion**: extracted canonical caption fan-out into `lib/content/captions.ts`, with deterministic `fast-check` invariants for complete platform keys, per-platform bounds, Unicode-safe truncation, and pillar-tag preservation; added a near-real pt-BR fixture exercised through parsing, compliance, and fan-out, plus E2E observable-output assertions, regression coverage, and a numeric throughput benchmark.
