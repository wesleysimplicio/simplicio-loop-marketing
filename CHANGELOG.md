...
- **Marketing extension conformance oracle** (issue #92): canonical `simplicio.loop-extension/v1` manifest, compatible/incompatible core gate, deterministic manifest and graph receipts, exactly-once fake-effect recovery/security tests, clean packed-install E2E, reproducible latency benchmark, rollback runbook, and Linux/Windows Node 18/20/22 CI matrix.
