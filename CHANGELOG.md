...
- **Loop core extension binding** (issue #93): publishes the pinned `simplicio.loop-extension/v1` `loop_marketing` manifest, hashed marketing context schemas, declarative stage/role/gate/resource bindings, core-owned budgets, receipt-derived views, and a fenced exactly-once effect adapter. Campaign startup now rejects an incompatible core before creating work, while doctor reports extension conformance.
