...
import { readHbi } from "../../lib/formats/binary.ts";
...
test("writeManifest: normalizes string providers into descriptors and defaults optional fields", () => {
  ...
  const written = readHbi<{ piece_id: string }>(join(out, "manifest.hbi"));
  assert.equal(written.piece_id, "piece-a");
});

test("writeManifest: accepts a direct .hbi target path and honors explicit overrides", () => {
  const dir = mkdtempSync(join(tmpdir(), "me-manifest-direct-"));
  const target = join(dir, "sub", "manifest.hbi");
  ...
  const written = readHbi<{ piece_id: string }>(target);
  assert.equal(written.piece_id, "p2");
});

test("runsLogPath: resolves data/runs.hbp under the given root (or cwd by default)", () => {
  const dir = mkdtempSync(join(tmpdir(), "me-runs-path-"));
  assert.equal(runsLogPath(dir), join(dir, "data", "runs.hbp"));
});
...
test("readRuns: returns an empty array for a missing file and rejects malformed binary", () => {
  const dir = mkdtempSync(join(tmpdir(), "me-runs-malformed-"));
  assert.deepEqual(readRuns(join(dir, "nope.hbp")), []);

  mkdirSync(join(dir, "data"), { recursive: true });
  const path = join(dir, "data", "runs.hbp");
  writeFileSync(path, "not-hbp");
  assert.throws(() => readRuns(path), /truncated|expected/);
});
