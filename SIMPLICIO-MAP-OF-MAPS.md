...
- [Demo run receipt (regenerated by `node scripts/demo-asolaria-loop.mjs`)](./.specs/strategy/campaigns/2026-Q3-asolaria-on-metal/DEMO-RUN.md)
- [Case study](./.specs/strategy/campaigns/2026-Q3-asolaria-on-metal/CASE-STUDY.md)
- [Simplicio on Metal landing page (static, deploy-ready)](./site/simplicio-on-metal/index.html)
- [Simplicio + Asolaria site section (static, deploy-ready)](./site/asolaria-integration/index.html)
- [Reductions benchmark script](./scripts/reductions-benchmark.mjs)
...
- deploying `site/simplicio-on-metal/index.html` at a real domain such as `simpleti.com.br/simplicio/on-metal`
- wiring `site/asolaria-integration/index.html` into an existing site's navigation
- recording and hosting demo media of `node scripts/demo-asolaria-loop.mjs`
- analytics from a public campaign run

The docs above are written so those external surfaces can be published later without changing engine core logic. The static pages under `site/` are already deploy-ready — only the hosting target is external.
